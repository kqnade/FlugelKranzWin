...
- Windowsの表示補正はDirectMode_009 SubmitLayerのフレーム姿勢から飛行変換を除きます。`correctFramePose` は既定false。Quest 2 / VDで回転は概ね動作、頭を動かした時の暗転は対策中です。MirrorViewの正常動作だけでHMD表示の検証完了としないでください。最新の確認範囲は `WINDOWS.md`。
